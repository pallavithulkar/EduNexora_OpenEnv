...
Range: 0.1 to 0.9 — safe for validator
"""
...
def grade_ranking(predicted_order: list, actual_marks: Dict[str, float]) -> float:
    ...
    return round(max(0.10, min(0.80, (correct / top_n) * 0.80)), 4)
...
def grade_prioritization(selected_unit: str, lag_scores: Dict[str, int]) -> float:
    ...
    if best_lag == 0:
        return 0.10
    return round(max(0.10, min(0.80, (selected_lag / best_lag) * 0.80)), 4)


def grade_topic_completion(progress_pct: float) -> float:
    return round(max(0.10, min(0.90, progress_pct / 100.0 * 0.90)), 4)
...
def grade_notification(notifications: list) -> float:
    ...
    return round(max(0.10, min(0.80, score * 0.80)), 4)
...
def compute_episode_score(rewards: list) -> Dict[str, Any]:
    if not rewards:
        return {"mean": 0.10, "max": 0.10, "min": 0.10, "total": 0.10, "count": 0}
    return {
        "mean":  round(max(0.10, min(0.90, sum(rewards) / len(rewards))), 4),
        "max":   round(max(0.10, min(0.90, max(rewards))), 4),
        "min":   round(max(0.10, min(0.90, min(rewards))), 4),
        "total": round(max(0.10, min(0.90, sum(rewards))), 4),
        "count": len(rewards),
    }
